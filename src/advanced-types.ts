...
/** It's a feature that allows us to define multiple function's signatures (params and returned values) for the same function and we use it when TS isn't able to infer the exact type of data that is returned from a function.
 *
...
/** for example, bellow we're using the type "Combinable" (created on line 30) as the type of the parameters, TS knows that the returned value can be a string or a number, but it cannot foresee which type will be.
 *
 * to define an "overload" to a function we simply need to "copy" the first line of the function declaration and define the types of the parameters and returned values we need.
 *
 * the signature of the implementation is not visible from the "outside", so when writing an overloaded function, we should always have two or more signatures above the implementation of the function.
 */
...
/** we could also define cases like this one bellow, but we should have optional params in our function and make sure to handle these cases properly. */
...
/** without the overload defined above we would get an error here, because TS just knew the returned value could be a number or a string, but not exactly what the type of the returned value really is, so we couldn't use the "split" method on it. */
// const result2 = addWithOverload("Ben ", "10");
// result2.split(" ");

/** here, to fix the error cited above, we could use type casting to make sure the return would be a string, but it may not be ideal deppending on the situation. */
// const result2 = addWithOverload("Ben ", "10") as string;

/** if we hover over the name of the function we see there are +3 overloads available. */
const result3 = addWithOverload("Ben ", "10");
console.log(result3);

/** after defining the overloads the error cited on line 227 is gone!
 *
 * but if we called addWithOverload with 2 numbers as arguments then ofc we wouldn't be able to call the string method ".split()" on it.
 */
