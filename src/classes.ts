 ...
 * that a child class must implement them, we shouldn't implement them on 
 * the parent class though, the implementation is the job of the child 
 * classes (see line 42).
*/ 
abstract class Department {
  ...
  */ 
  constructor(protected readonly id: string, public name: string) {
    /**
     * Using the "access modifier" in front of the parameter we can also 
     * get rid of the 2 lines bellow since TS will create them for us.
    */ 
    // this.id = id;
    // this.name = name;
    ...
    */ 
    ...
    /**
     * If we really want to access static properties (or methods) inside 
     * this class where they were created or from an instance of this 
     * class, we could fix the code above by dropping the "this" keyword 
     * and accessing it directly from the class name.
    */ 
  ...
  /**
   * Here we just initialized the method, but it doesn't even have a body 
   * "{}" and that's because it should be implemented by the child classes.
  */ 
...
/**
 * Bellow we were able to access the "static" method and property directly 
 * on the class, without the need of instantiating it.
*/ 
...
/**
 * it stopped working later in the course, because we added the "abstract" 
 * modifier in this class, so I had to comment all of it out.
*/ 
 ...
 * employees adn we shouldn't manipulate the "employees" array directly.
*/ 
...
/**
 * Here we get an error because TS detects that the "this" keyword of the 
 * copy is of the type "Department" and it lacks the some fields/propertys 
 * and methods (it doesn't have the same signature so to speak), without 
 * doing what we did above with the "pseudo parameter" TS wouldn't show an 
 * error. To fix the error we would just need to add all the properties 
 * and methods of the Department class to this copy.
*/ 
// accountingCopy.describe();

/** we can only inherit/extend from 1 class, it's not possible to inherit 
 * from multiple classes.
*/ 
class ITDepartment extends Department {
  /**
   * Here we could also have used the shortcut cited in line 10, but for 
   * the sake of the example we'll keep it this way.
  */ 

  admins: string[];
  constructor(id: string, admins: string[]) {
    /** 
     * Whenever we add a constructor in a class that inherit from another 
     * class we have to use the "super()" function, this function calls 
     * the constructor of the "base/parent" class.
     * 
     * If we hover over "super" we'll see that it requires an id and a 
     * name, like the constructor of the "Department" class, in this case 
     * we passed the "id", but hard-coded the department name, since it 
...
class SalesDepartment extends Department {
  ...
  /**
   * Here basically what we're doing is storing a SalesDepartment instance 
   * in this variable, so we can use it later to create only one instance 
   * of this class (OOP singleton pattern, see line 143).
  */ 
   ...
   * help to make our code clearer, because at a glance we can tell what 
   * is a getter, what is a setter and what is a regular method that do 
   * something else in our class.
   * 
   ...
   * and provided to the user, thus making it easier to verify and debug. 
   * 
   * Getter methods must NOT have any parameters, this syntax binds an 
   * object property to a function that will be called when that property 
   * is looked up.
  */ 
  get mostRecentReport() {
    if (this.lastReport) {
       ...
       * 
       * The object's property's value does not get calculated until it is 
       * accessed.
      */ 
  ...
  /**
   * Setter methods must have 1 parameter and are used to set/change the 
  ...
  */ 
   ...
   * singleton pattern, that is a class that can ONLY be instantiated once 
   * (see lines 113, 149). 
  */ 
  ...
  static getInstance() {
     ...
     * 
     * Inside the class we can use the "new" keyword to instatiate itself, 
     * even the constructor being private (see line 143 and 188).
    */ 
  ...
  addEmployee(name: string) {
     ...
     * parent class was "private" (line 8) and even a child class cannot 
     * access a private property, so we had to change it to "protected" 
     * and now this class can also have access to that  property.
    */ 
...
/**
 * The instantiation bellow stopped working when we turned the constructor 
 * of the SalesDepartment class private to make a singleton pattern.
*/ 
 ...
 * instance as above. The singleton pattern is not something we use all 
 * the time, but it's useful to know that it's possible to create it in TS 
 * thanks to the private constructor.
*/ 
